...
int __vfprintf_chk(FILE * stream, int flag, const char * format, va_list arg){
    ...
    if(cur==0){
        ...
        chk = my_decrypt(mypri_key, actual_block, 16, my_ek, my_ekl, my_iv, decrypt_block, &block_len);
        char append[4096];
        ...
        fseeko(stream, block_init, SEEK_SET);
        fwrite(output, 1, cipher_len, stream);
        fseeko(stream, block_len + strlen(buffer), SEEK_SET);
    }
    else{
        ...
        if(cur==16){
            ...
            chk = my_decrypt(mypri_key, actual_block, 32, my_ek, my_ekl, my_iv, decrypt_block, &block_len);
            printf("block_len = %ld\n", chk1);
            char append[4096];
            memset(append, 0, 4096);
            memcpy(append, decrypt_block, block_len);
            ...
            fwrite(output, 1, cipher_len, stream);
            fseeko(stream, block_len + strlen(buffer), SEEK_SET);
            printf("mlen = %d pointer = %ld\n", cipher_len, block_len+strlen(buffer));
        }
        else if(cur%16){
    ...
    //teste
    //fprintf_ocall((uint64_t) stream, output, &result);  //teste
    char output1[4096];
    int plain_size;
    memset(output1, 0, 4096);
    chk = my_decrypt(mypri_key, output, cipher_len, my_ek, my_ekl, my_iv, output1, &plain_size);
    printf("output 1 = %s\n", output1);
    return result;
}
...
int __fprintf_chk (FILE * stream, int flag, const char * format, ...){
    ...
    if(cur==0){
        ...
        if(cur==16){
            ...
            fread_ocall(actual_block, 1, 32, (uint64_t) stream, 16, &chk1);
            if(chk1 != 32)   printf("Error fread_ocall!%d\n", -1);
            chk = my_decrypt(mypri_key, actual_block, 32, my_ek, my_ekl, my_iv, decrypt_block, &block_len);
            char append[4096];
            memset(append, 0, 4096);
            ...
            fread_ocall(actual_block, 1, 32, (uint64_t) stream, 32, &chk1);
            if(chk1 != 32)  printf("Error fread_ocall!%d\n", -1);
            chk = mydecrypt(mypri_key, actual_block, 32, my_ek, my_ekl, block_iv, decrypt_block, &block_len);
            char append[4096];
            memset(append, 0, 4096);
            memcpy(append, decrypt_block, block_len);
            memcpy(append+block_len, buffer, strlen(buffer));
            ...
            fread_ocall(actual_block, 1, 16, (uint64_t) stream, 16, &chk1);
            if(chk1 != 16)  printf("Error fread_ocall!%d\n", -1);
            chk = mydecrypt(mypri_key, actual_block, 16, my_ek, my_ekl, block_iv, decrypt_block, &block_len);
            char append[4096];
            memset(append, 0, 4096);
            memcpy(append, decrypt_block, block_len);
            memcpy(append+block_len, buffer, strlen(buffer));
