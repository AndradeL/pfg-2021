...
int main(int argc, char *argv[])
{
    ...
    // set up I/O files 
    vinit=sf_input ("rsf/smvel.rsf");   // initial velocity model, unit=m/s 
    shots=sf_input("rsf/shots.rsf"); // recorded shots from exact velocity model
    vupdates=sf_output("rsf/out.rsf"); // updated velocity in iterations  
    grads=sf_output("rsf/grads.rsf");  // gradient in iterations  
    illums=sf_output("rsf/illums.rsf");// source illumination in iterations 
    objs=sf_output("rsf/objs.rsf");// values of objective function in iterations 
    ...
    if (!sf_getint("niter",&niter))   niter=3;	// number of iterations 
    ...
    {
	if(verb) {
	    //start=oe_get_time();// record starting time
	...
	if(verb) {// output important information at each FWI iteration 
	    ...
	    //stop=oe_get_time();// record ending time 
	    //sf_warning("iteration %d finished: %f (s)",iter+1, ((float)(stop-start)/1000));
	}
    }
